...
const mailTransport = emailOtpConfigured ?
...
function requireCustomer(req, res, next) {
    ...
    try {
        ...
        return res.status(401).json({
            message: "Customer login expired. Verify your email again."
        });
    }
}
...
function validateDeliveryData(x) {
    ...
    if (!x.customerName ||
        ...
        return {
            ok: false,
            message: "Customer name, email, mobile number and address are required"
        };
    }
...
async function start() {
    ...
    await Promise.all([
        ...
        emailOtps.createIndex({ emailNormalized: 1 }, { unique: true }),

        emailOtps.createIndex({ expiresAt: 1 }, { expireAfterSeconds: 0 }),

        orders.createIndex({ razorpayPaymentId: 1 }, { unique: true, sparse: true }),
        ...
        try {
            ...
            if (
                ...
                return res.status(400).json({
                    acknowledged: false,
                    message: "Online payments must be verified before the order is placed"
                });
            }
        ...
        async(req, res) => {
            try {
                ...
                if (!safeEqual(
                    ...
                    return res.status(400).json({
                        acknowledged: false,
                        message: "Payment verification failed. Order was not placed."
                    });
                }
                ...
                if (
                    ...
                    return res.status(400).json({
                        acknowledged: false,
                        message: "Payment amount mismatch. Order was not placed."
                    });
                }
                ...
                if (
                    ...
                    return res.status(409).json({
                        acknowledged: false,
                        message: "Payment is not captured yet. Order was not placed."
                    });
                }
                ...
                if (!product) {
                    return res.status(409).json({
                        acknowledged: false,
                        message: "Payment was received but the product record is unavailable. Contact the store."
                    });
                }

                const onlineMethod = ["UPI", "Card"].includes(
                ...
                const order = {
                    ...
                    productName: attempt.productSnapshot &&
                        attempt.productSnapshot.name ?
                        attempt.productSnapshot.name :
                        product.name,

                    price: Number(attempt.amount) /
                        100 /
                        Math.max(
                            1,
                            Number(
                ...
                await paymentAttempts.updateOne({
                    ...
                    $set: {
                        status: "placed",
                        razorpayPaymentId,
                        placedOrderId: result.insertedId,
                        updatedAt: new Date()
                    }
                });
                ...
                res.status(500).json({
                    acknowledged: false,
                    message: "Payment could not be verified. Order was not placed."
                });
            }
        }
    );
    ...
    app.post(
        ...
        async(req, res) => {
            try {
                ...
                await emailOtps.updateOne({ emailNormalized: email }, {
                    $set: {
                        emailNormalized: email,
                ...
                try {
                    await mailTransport.sendMail({
                        ...
                        text: `Your MAHALAKSHIMI STORE OTP is ${code}. ` +
        ...
        async(req, res) => {
            try {
                ...
                if (!safeEqual(expected, record.otpHash)) {
                    await emailOtps.updateOne({ emailNormalized: email }, { $inc: { attempts: 1 } });
                    return res.status(401).json({
                ...
                const token = jwt.sign({
                    ...
                    CUSTOMER_JWT_SECRET, { expiresIn: "12h" }
                );
            ...
            try {
                ...
                if (!order) {
                    return res.status(404).json({
                        message: "This order is not available for your verified email address"
                    });
                }
